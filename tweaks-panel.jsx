...
function TweaksPanel({ title = 'Tweaks', noDeckControls = false, children }) {
  // 独立部署版：默认隐藏。`#tweaks` hash 或键盘 Shift+T 调出（避免普通访客看到调试面板）。
  const [open, setOpen] = React.useState(
    () => typeof window !== 'undefined' && window.location.hash === '#tweaks'
  );
  React.useEffect(() => {
    const onHash = () => setOpen(window.location.hash === '#tweaks');
    const onKey = (e) => {
      if ((e.key === 'T' || e.key === 't') && e.shiftKey && !e.metaKey && !e.ctrlKey) {
        const tag = (e.target && e.target.tagName) || '';
        if (tag === 'INPUT' || tag === 'TEXTAREA') return;
        const cur = window.location.hash === '#tweaks';
        history.replaceState(null, '', cur ? location.pathname + location.search : '#tweaks');
        setOpen(!cur);
      }
    };
